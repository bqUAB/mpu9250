...
void MPU9250::MPU9250SelfTest(float* destination){
  ...
  /* Enable self test on all three axes and set gyro range to +/- 250
   *degrees/s */
  writeByte(GYRO_CONFIG,  0xE0);
  /* Enable self test on all three axes and set accelerometer range to
   * +/- 2 g */
...
void MPU9250::calibrateMPU9250(float* gyroBias, float* accelBias){
   ...
   * which are reset to zero upon device startup. Divide by 4 to get 32.9 LSB
   ...
   * bias scaled to 2048 LSB/g (16 g full scale) */
...
void MPU9250::initAK8963(float* destination){
  ...
  usleep(10*1000);
  writeByte(AK8963_CNTL, 0x0F); // Enter Fuse ROM access mode
  usleep(10*1000);
  /* Read the x-, y-, and z-axis calibration values */
  readBytes(AK8963_ASAX, 3, &rawData[0]);
  ...
  writeByte(AK8963_CNTL, 0x00); // Power down magnetometer
  usleep(10*1000);
  /* Configure the magnetometer for continuous read and highest resolution, set
   * Mscale bit 4 to 1 (0) to enable 16 (14) bit resolution in CNTL register,
   * and enable continuous mode data acquisition Mmode (bits [3:0]), 0010 for
   * 8 Hz and 0110 for 100 Hz sample rates */

  /* Set magnetometer data resolution and sample ODR */
  writeByte(AK8963_CNTL, Mscale << 4 | Mmode);
  usleep(10*1000);
}
